...
def load_dataset(test_num=100):
    ...
    train_data = pd.read_csv("/data/fjsdata/ctKngBase/kbcc_trainset.csv", sep='|', low_memory=False, dtype={'csr':int, 'ke':int, 'num':float})
    train_data =train_data[['csr','ke']]
    user_num = train_data['csr'].max() + 1
    item_num = train_data['ke'].max() + 1
    ...
    test_data = pd.read_csv("/data/fjsdata/ctKngBase/kbcc_testset.csv", sep='|', low_memory=False, dtype={'csr':int, 'ke':int, 'num':float})
    test_data =test_data[['csr','ke']]
    test_data = test_data.values.tolist()#one postive item ,99 negative items
...
"""
nohup python -u NCFTorchGPU-kb.py > ncftorchgpu-kb.log  &
K          Iterations            HitRatio                NDCG
  8                  20            0.981793            0.869697
 16                  20            0.980325            0.871115
 32                  20            0.979346            0.867237
 64                  20            0.981989            0.871263
"""
